...
class TestRemoteciFlavors(utils.SchemaTesting):
    ...
    def test_post_extra_data(self):
        ...
        super(TestRemoteciFlavors, self).test_post(data, data_expected)

    def test_post_missing_data(self):
        errors = utils.generate_errors('topic_id', 'name')
        super(TestRemoteciFlavors, self).test_post_missing_data(errors)

    def test_post_invalid_data(self):
        invalids, errors = TestRemoteciFlavors.\
            generate_invalids_and_errors()
        super(TestRemoteciFlavors, self).test_post_invalid_data(invalids,
                                                                errors)
    ...
    def test_post(self):
        ...
        super(TestRemoteciFlavors, self).test_post(self.data, data_expected)

    def test_put_extra_data(self):
        super(TestRemoteciFlavors, self).test_put_extra_data(
            {}, data_extra={utils.TOPIC[0]: utils.TOPIC[1]})

    def test_put_invalid_data(self):
        invalids, errors = TestRemoteciFlavors.\
            generate_invalids_and_errors()
        super(TestRemoteciFlavors, self).test_put_invalid_data(invalids,
                                                               errors)
    ...
    def test_put(self):
        ...
        del data['topic_id']
        super(TestRemoteciFlavors, self).test_put(data, data)
