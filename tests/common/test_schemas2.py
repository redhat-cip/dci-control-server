...
def test_check_json_is_valid_check_integer_type():
    ...
    with pytest.raises(DCIException) as e:
        ...
        errors = e.value.payload["errors"]
        assert len(errors) == 1
        assert errors[0] == "integer: 'not an integer' is not of type 'integer'"  # noqa
...
def test_check_json_is_valid_check_positive_integer_type():
    ...
    with pytest.raises(DCIException) as e:
        ...
        errors = e.value.payload["errors"]
        assert len(errors) == 1
        assert errors[0] == "positive_integer: -1 is less than the minimum of 1"  # noqa
...
def test_check_json_is_valid_check_string_integer_type():
    ...
    with pytest.raises(DCIException) as e:
        ...
        errors = e.value.payload["errors"]
        assert len(errors) == 1
        assert errors[0] == "string_integer: 'not an string_integer' is not of type 'string_integer'"  # noqa
...
def test_check_json_is_valid_check_positive_string_integer_type():
    ...
    with pytest.raises(DCIException) as e:
        ...
        errors = e.value.payload["errors"]
        assert len(errors) == 1
        assert errors[0] == "positive_string_integer: '-1' is not of type 'positive_string_integer'"  # noqa

    with pytest.raises(DCIException) as e:
        check_json_is_valid(schema, {"positive_string_integer": "0"})
        errors = e.value.payload["errors"]
        assert len(errors) == 1
        assert errors[0] == "positive_string_integer: '0' is not of type 'positive_string_integer'"  # noqa
