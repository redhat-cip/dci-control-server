...
def test_add_test_to_topic_and_get(admin, test_id, team_user_id):
    # create a topic
    data = {'name': 'rname', 'team_id': team_user_id}
    pr = admin.post('/api/v1/topics', data=data).data
    pr_id = pr['topic']['id']

    # attach a test to topic
    url = '/api/v1/topics/%s/tests' % pr_id
    add_data = admin.post(url, data={'test_id': test_id}).data
    assert add_data['topic_id'] == pr_id
...
def test_delete_test_from_topic(admin, test_id, team_user_id):
    # create a topic
    ...
    test_from_topic = admin.get(
        '/api/v1/topics/%s/tests' % pr_id).data
    assert test_from_topic['_meta']['count'] == 1
