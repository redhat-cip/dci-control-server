...
def test_get_all_jobs_with_where(admin, job_user_id, team_user_id):
...
def test_get_job_by_id(admin, job_user_id):
    job = admin.get('/api/v1/jobs/%s' % job_user_id)
    assert job.status_code == 200

    job = job.data
    assert job['job']['id'] == job_user_id
...
def test_embed_with_subkey_in_where(admin, job_user_id, topic_id):
...
def test_get_jobs_with_schedule(admin, remoteci_user, topic_user_id,
                                remoteci_user_id, components_user_ids):
    # schedule a job
    data = {'remoteci_id': remoteci_user_id, 'topic_id': topic_user_id}
    job = remoteci_user.post('/api/v1/jobs/schedule', data=data)
    assert job.status_code == 201
    job_id = job.data['job']['id']
...
@pytest.mark.usefixtures('job_user_id')
...
def test_get_job_as_user(user, job_user_id):
    job = user.get('/api/v1/jobs/%s' % job_user_id)
    assert job.status_code == 200


def test_delete_job_as_user(user, remoteci_user_id, job_user_id):
    job = user.get('/api/v1/jobs/%s' % job_user_id)
    job_etag = job.headers.get("ETag")
...
def test_create_file_for_job_user_id(user, job_user_id, remoteci_user_id,
                                     components_user_ids):
    with mock.patch(SWIFT, spec=Swift) as mock_swift:
