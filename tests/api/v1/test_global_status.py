...
def test_global_status(admin, remoteci_context, remoteci_id, product):
    ...
    remoteci_context.post('/api/v1/jobs',
                          data={'components': [component['id']]})
    global_status = admin.get('/api/v1/global_status').data['data']
    print(global_status)
    stats = global_status['RH7-RHOS-12.0 2016-11-13.1']
    assert len(stats['jobs']) == 1
    assert stats['jobs'][0]['status'] == 'success'
    assert stats['jobs'][0]['remoteci']['id'] == remoteci_id
