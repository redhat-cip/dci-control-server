...
def test_create_get_analytics(admin, job_user_id):
    ...
    anc = admin.post('/api/v1/jobs/%s/analytics' % job_user_id,
                     data={'name': 'rhsm pool issue',
                           'type': 'infrastructure',
                           'url': 'http://bugzilla/1',
                           'data': {'root_cause': 'pool not found'}})
    assert anc.status_code == 201
    analytics = admin.get('/api/v1/jobs/%s/analytics' % job_user_id)
    c_analytic = analytics.data['analytics'][0]

    assert c_analytic['name'] == 'rhsm pool issue'
    assert c_analytic['type'] == 'infrastructure'
    assert c_analytic['url'] == 'http://bugzilla/1'
    assert c_analytic['data'] == {'root_cause': 'pool not found'}
...
def test_get_several_analytics(admin, job_user_id):
    ...
    for i in range(3):
        job = admin.post('/api/v1/jobs/%s/analytics' % job_user_id,
                         data={'name': 'rhsm pool issue %s' % i,
                               'type': 'infrastructure',
...
def test_put_analytic(admin, job_user_id):
    ...
    res = admin.put('/api/v1/jobs/%s/analytics/%s' % (job_user_id, anc_id),
