...
def test_attach_issue_to_job(admin, job_user_id):
    with mock.patch(GITHUB_TRACKER, spec=requests) as mock_github_request:
        ...
        issue = admin.post('/api/v1/jobs/%s/issues' % job_user_id, data=data).data  # noqa
        result = admin.get('/api/v1/jobs/%s/issues' % job_user_id).data
...
def test_unattach_issue_from_job(admin, job_user_id):
    with mock.patch(GITHUB_TRACKER, spec=requests) as mock_github_request:
        mock_github_result = mock.Mock()
        mock_github_request.get.return_value = mock_github_result
        ...
        result = admin.post('/api/v1/jobs/%s/issues' % job_user_id, data=data)
        issue_id = result.data['issue']['id']
        result = admin.get('/api/v1/jobs/%s/issues' % job_user_id).data
        assert result['_meta']['count'] == 1
        admin.delete('/api/v1/jobs/%s/issues/%s' % (job_user_id, issue_id))
        result = admin.get('/api/v1/jobs/%s/issues' % job_user_id).data
...
def test_github_tracker(admin, job_user_id):
    with mock.patch(GITHUB_TRACKER, spec=requests) as mock_github_request:
        mock_github_result = mock.Mock()
        mock_github_request.get.return_value = mock_github_result
        ...
        admin.post('/api/v1/jobs/%s/issues' % job_user_id, data=data)
        result = (
            admin.get('/api/v1/jobs/%s/issues' % job_user_id).data['issues'][0]
        )
...
def test_github_tracker_with_private_issue(admin, job_user_id):
    with mock.patch(GITHUB_TRACKER, spec=requests) as mock_github_request:
        mock_github_result = mock.Mock()
        mock_github_request.get.return_value = mock_github_result
...
def test_bugzilla_tracker(admin, job_user_id):
...
def test_bugzilla_tracker_with_non_existent_issue(admin, job_user_id):
    with mock.patch(BUGZILLA_TRACKER, spec=requests) as mock_bugzilla_request:
        mock_bugzilla_result = mock.Mock()
        mock_bugzilla_request.get.return_value = mock_bugzilla_result
