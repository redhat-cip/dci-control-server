...
def test_purge_resource(client_admin, rhel_product):
    data = {
        "name": "tname",
    ...
    to_purge = client_admin.get("/api/v1/topics/purge").data
    assert len(to_purge["topics"]) == 1

    to_purge = client_admin.post("/api/v1/topics/purge")
...
def test_purge_resource_ORM(client_admin, team_admin_id):
    feeder = client_admin.post(
    ...
    feeders_to_purge = client_admin.get("/api/v1/feeders/purge").data["feeders"]
    assert len(feeders_to_purge) == 1
    assert feeders_to_purge[0]["id"] == feeder["id"]

    purge = client_admin.post("/api/v1/feeders/purge")
...
def test_where_name_case_insensitive(client_user1, team1_id):
    remoteci = client_user1.post(
        "/api/v1/remotecis", data={"name": "My remoteci", "team_id": team1_id}
    ).data["remoteci"]
    remoteci_id = remoteci["id"]

    remotecis = client_user1.get("/api/v1/remotecis?where=id:%s" % remoteci_id).data[
        "remotecis"
    ]
    assert len(remotecis) == 1
    assert remotecis[0]["id"] == remoteci_id

    remotecis = client_user1.get("/api/v1/remotecis?where=name:My%20remote*").data[
        "remotecis"
    ]
    assert len(remotecis) == 1
    assert remotecis[0]["id"] == remoteci_id
    ...
    remotecis = client_user1.get("/api/v1/remotecis?where=name:My%20remoteci").data[
        "remotecis"
    ]
    assert len(remotecis) == 1
    assert remotecis[0]["id"] == remoteci_id
...
def test_where_with_different_column_types(client_user1):
    request = client_user1.get("/api/v1/jobs?where=name:foo")
    assert request.status_code == 200

    request = client_user1.get(
        "/api/v1/jobs?where=id:d781d517-643d-4a35-af35-4702a12db9ae"
    )
    assert request.status_code == 200
    ...
    request = client_user1.get("/api/v1/jobs?where=duration:134")
    assert request.status_code == 200

    request = client_user1.get("/api/v1/jobs?where=tags:134")
    assert request.status_code == 200

    request = client_user1.get("/api/v1/jobs?where=state:active")
    assert request.status_code == 200
...
def test_where_with_forbidden_column_names(client_user1):
    ...
    request = client_user1.get("/api/v1/jobs?where=cert_fp:brute_force")
    assert request.status_code == 400
