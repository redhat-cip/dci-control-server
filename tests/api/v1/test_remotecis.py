...
def test_generate_keys(user, team_user_id, remoteci_user_id, cakeys):
    ...
    keys = user.put(
        ...
        headers={"If-match": rci["remoteci"]["etag"]},
    ).data
...
def test_put_remoteci_as_user(user, team_user_id, remoteci_id, admin):
    ...
    remoteci = user.get("/api/v1/remotecis/%s" % remoteci.data["remoteci"]["id"]).data[
        "remoteci"
    ]
    ...
    remoteci_etag = remoteci.headers.get("ETag")
    remoteci_put = user.put(
        "/api/v1/remotecis/%s" % remoteci_id,
        data={"name": "nname"},
        headers={"If-match": remoteci_etag},
    )
...
def test_delete_remoteci_as_user(user, team_user_id, admin, remoteci_id):
    ...
    remoteci_etag = remoteci.headers.get("ETag")
    remoteci_delete = user.delete(
        "/api/v1/remotecis/%s" % remoteci_id, headers={"If-match": remoteci_etag}
    )
...
def test_change_remoteci_state(admin, remoteci_id):
    ...
    data = {"state": "inactive"}
    r = admin.put(
        "/api/v1/remotecis/" + remoteci_id, data=data, headers={"If-match": t["etag"]}
    )
...
def test_success_detach_myself_from_remoteci_in_team(user, user_id, remoteci_user_id):
    ...
    r = user.delete("/api/v1/remotecis/%s/users/%s" % (remoteci_user_id, user_id))
