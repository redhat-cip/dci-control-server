...
def test_success_delete_feeder_authorized_users(admin, epm, feeder, team_user_id):
    ...
    admin.delete(
        "/api/v1/feeders/%s" % feeder["id"], headers={"If-match": feeder["etag"]}
    )
    epm.delete(
...
def test_success_refresh_secret_feeder_authorized_users(admin, epm, feeder):
    """Test to ensure user with proper permissions can update feeders."""
