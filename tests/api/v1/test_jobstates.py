...
def test_get_all_jobstates_with_embed(admin, job_id, team_admin_id):
    ...
    jobstate_1 = admin.post('/api/v1/jobstates', data=data).data

    files.post_file(admin, jobstate_1['jobstate']['id'],
    ...
    assert len(js_1['files']) == 1
    assert js_1['files'][0]['name'] == 'foo'

    assert 'team_id' not in js_2
    assert 'team' in js_2
    assert js_2['team']['id'] == team_admin_id
    assert len(js_2['files']) == 0
