...
def test_create_files(admin, jobstate_user_id, team_user_id):
...
def test_get_all_files_with_embed(admin, jobstate_user_id, team_admin_id,
                                  job_user_id):
...
# TODO(all): test isolation with another user team insted of admin team
@pytest.mark.usefixtures('file_user_id')
...
# TODO(all): create another user team to test isolation insted of admin team
def test_get_file_as_user(user, jobstate_user_id):
    with mock.patch(SWIFT, spec=Swift) as mock_swift:
...
# TODO(all): create another user team to test delete isolation instead of admin
def test_delete_file_as_user(user, jobstate_user_id):
...
def test_get_file_content(admin, jobstate_user_id):
    with mock.patch(SWIFT, spec=Swift) as mock_swift:
        ...
        file_id = post_file(admin, jobstate_user_id, FileDesc('foo', data))
