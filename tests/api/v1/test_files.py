...
def test_delete_file_by_id(admin, jobstate_id):
    ...
    assert gfile['file']['state'] == 'archived'
...
def test_change_file_state(admin, file_id):
    ...
    current_file = admin.get('/api/v1/files/' + file_id).data['file']
    assert current_file['state'] == 'inactive'
...
def test_change_file_to_invalid_state(admin, file_id):
    ...
    assert current_file['file']['state'] == 'active'
