...
def test_add_file_to_component(admin, topic_id):
    with mock.patch(SWIFT, spec=Swift) as mock_swift:
        ...
        cts = admin.get('/api/v1/components/%s?embed=component_files' % ct_1['id']).data
        assert len(cts['component']['component_files']) == 1
        assert cts['component']['component_files'][0]['size'] == 1
