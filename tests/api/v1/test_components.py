...
def test_recreate_components_with_same_name_on_same_topics(admin, topic_id):
    ...
    for n in range(3):
        ...
        result = admin.delete("/api/v1/components/%s" % result.data["component"]["id"])
...
def test_get_last_components_by_type(engine, admin, topic):
    ...
    last_components = components.get_last_components_by_type(
        ["puddle_osp"], topic_id=topic["id"], db_conn=engine
    )
...
def test_verify_and_get_components_ids(engine, admin, topic, topic_user_id):
    ...
    with pytest.raises(dci_exc.DCIException):
        components.verify_and_get_components_ids(
            topic["id"], [], ["puddle_osp"], db_conn=engine
        )

    with pytest.raises(dci_exc.DCIException):
        components.verify_and_get_components_ids(
            topic["id"], [str(uuid.uuid4())], ["puddle_osp"], db_conn=engine
        )
    ...
    with pytest.raises(dci_exc.DCIException):
        components.verify_and_get_components_ids(
            topic_user_id, [c1, c2, c3], ["type_1", "type_2", "type_3"], db_conn=engine
        )

    cids = components.verify_and_get_components_ids(
        topic_user_id, [c1, c3], ["type_1", "type_2"], db_conn=engine
    )
...
def test_add_tags_components(admin, components_ids):
    pt = admin.post(
        "/api/v1/components/%s/tags" % (components_ids[0]), data={"name": "my_tag_1"}
    )
    ...
    pt = admin.post(
        "/api/v1/components/%s/tags" % (components_ids[0]), data={"name": "my_tag_2"}
    )
    assert pt.status_code == 201
...
def test_filter_component_by_tag(
    admin, remoteci_context, components_user_ids, topic_user_id
):
    admin.post(
        "/api/v1/components/%s/tags" % components_user_ids[0], data={"name": "tag_1"}
    )
    admin.post(
        "/api/v1/components/%s/tags" % components_user_ids[0], data={"name": "common"}
    )

    admin.post(
        "/api/v1/components/%s/tags" % components_user_ids[1], data={"name": "tag_2"}
    )
    admin.post(
        "/api/v1/components/%s/tags" % components_user_ids[1], data={"name": "common"}
    )

    res = admin.get(
        "/api/v1/topics/%s/components?where=tags:common,tags:tag_1" % topic_user_id
    )
    assert len(res.data["components"]) == 1
    assert "tag_1" in res.data["components"][0]["tags"]
    assert "tag_2" not in res.data["components"][0]["tags"]

    res = admin.get("/api/v1/topics/%s/components?where=tags:common" % topic_user_id)
...
def test_add_and_delete_tags_teams_components(user, team_user_id, topic_user_id):
    ...
    pt = user.post("/api/v1/components/%s/tags" % pc_id, data={"name": "my_tag_1"})
    assert pt.status_code == 201

    pt = user.post("/api/v1/components/%s/tags" % pc_id, data={"name": "my_tag_1"})
    assert pt.status_code == 201

    pt = user.post("/api/v1/components/%s/tags" % pc_id, data={"name": "my_tag_2"})
    assert pt.status_code == 201
    ...
    dt = user.delete("/api/v1/components/%s/tags" % pc_id, data={"name": "my_tag_2"})
    assert dt.status_code == 204
