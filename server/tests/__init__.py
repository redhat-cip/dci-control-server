...
class DCITestCase(testtools.TestCase):
    ...
    def setUpClass(cls):
        ...
        subprocess.call(['psql', '--quiet',
                         ...
                         'template1'])
        time.sleep(0.3)
        res = subprocess.call(['psql', '-h', cls._db_dir, '-c', "\"INSERT INTO teams (name) VALUES ('admin');\"", 'template1'])
        if res:
            print("error1")
            sys.exit(1)
        res = subprocess.call(['psql', '-h', cls._db_dir, '-c', "\"INSERT INTO roles (name) VALUES ('admin');\"", 'template1'])
        if res:
            print("error2")
            sys.exit(1)

        res = subprocess.call(['psql', '-h', cls._db_dir, '-c', "\"INSERT INTO users (name, password, team_id) VALUES ('admin', crypt('admin', gen_salt('bf', 8)), (SELECT id FROM teams WHERE name='admin'));\"", 'template1'])
        if res:
            print("error3")
            sys.exit(1)
        time.sleep(0.3)
