...
class TestAuth(object):
    ...
    def test_team_isolation(self, admin, company_b_user, company_a_user):
        ...
        jobstates = utils.create_jobstates(company_b_user, job.data['id'])
        a_file = utils.create_file(company_b_user, jobstates.data['id'])

        assert a_file.status_code == 201

        assert company_a_user.get('/api/files/%s' % a_file.data['id']).status_code == 404
        assert company_b_user.get('/api/files/%s' % a_file.data['id']).status_code == 200
        assert admin.get('/api/files/%s' % a_file.data['id']).status_code == 200
