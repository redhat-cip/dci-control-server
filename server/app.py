...
class DciApp(Eve):
    ...
    def __init__(self, dci_model, **kwargs):
        super(DciApp, self).__init__(**kwargs)

        DciApp._DCI_MODEL = dci_model
        DciApp._DCI_MODEL.metadata.bind = DciApp._DCI_MODEL.engine
        self._db = self.data.driver
        self._db.Model = DciApp._DCI_MODEL.base
    ...
    def aggregate_job_data(response):
        session = DciApp._DCI_MODEL.get_session()
        data = {}
        job = session.query(DciApp._DCI_MODEL.base.classes.jobs).\
            get(response['id'])
        my_datas = (
    ...
    def get_jobs_extra(response):
        ...
        for job in response["_items"]:
            ...
            # Get the jobstate
            Jobstates = DciApp._DCI_MODEL.base.classes.jobstates
            ...
            Remotecis = DciApp._DCI_MODEL.base.classes.remotecis
    ...
    def get_versions_extra(response):
        ...
        for version in response["_items"]:
            ...
            for testversion in testversions:
                ...
                Jobs = DciApp._DCI_MODEL.base.classes.jobs
    ...
    def init_hooks(self):
        ...
        self.on_insert_jobs += DciApp.pick_jobs
        self.on_fetched_item_jobs += DciApp.aggregate_job_data
        self.on_fetched_resource_jobs += DciApp.get_jobs_extra
        self.on_fetched_resource_versions += DciApp.get_versions_extra
...
if __name__ == "__main__":
    ...
    app = DciApp(dci_model, settings=settings, validator=ValidatorSQL,
                 data=SQL, auth=basic_auth)
    app.init_hooks()
    site_map()
    app.run(debug=True)
