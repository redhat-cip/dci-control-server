...
2.  ensure that a postgresql database is running and accessible on the URI defined in the `src/settings.py` (if no database is running see [database installation](#database-installation))
3.  ensure that an elastic search is running and accessible on the URI defined in the `src/settings.py` (if no elastic search is running see [elastic search installation](elastic-search-installation))
