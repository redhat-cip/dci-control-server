...
app.factory('CommonCode', function($window, Restangular) {
    ...
    'aggregateJobInfo': function(job) {
        return Restangular.one('testversions', job.testversionId).get().then(
        function(testVersion) {
            Restangular.one('remotecis', job.remoteciId).get().
            then(function(data) {job['remoteci'] = data;});
            Restangular.one('tests', testVersion.testId).get().
            then(function(data) {job['test'] = data;});
            Restangular.one('versions', testVersion.versionId).get().
            then(function(data) {
                job['version'] = data;
