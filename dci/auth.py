...
# in place. Method calling this function should be decorated
# with an @auth.has_permissions(PERM) decorator. Meanwhile,
# for the migration to be seemless, we need to have this method
# around
...
def is_admin(user):
    return user['role_id'] == get_role_id('SUPER_ADMIN')


def is_admin_user(user, team_id):
    return str(user['team_id']) == str(team_id) and \
        user['role_id'] in [get_role_id('SUPER_ADMIN'), get_role_id('ADMIN')]


def is_in_team(user, team_id):
    return str(user['team_id']) == str(team_id) or is_admin(user)
        ...
        def decorated(*args, **kwargs):
            for mechanism in [BasicAuthMechanism(flask.request)]:
                if mechanism.is_valid():
                    ...
                    user_permissions = [dict(row)['label'] for row in rows]
