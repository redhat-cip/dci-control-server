...
def test_get_all_tests(admin):
    ...
    for i in range(5):
        ...
        created_ts_ids.append(pct['test']['id'])
    created_ts_ids.sort()

    db_all_ts = admin.get('/api/v1/tests').data
    db_all_ts = db_all_ts['tests']
    db_all_ts_ids = [db_t['id'] for db_t in db_all_ts]
    db_all_ts_ids.sort()

    assert db_all_ts_ids == created_ts_ids
...
def test_get_all_tests_with_where(admin):
    ...
    db_t = admin.get('/api/v1/tests?where=id:%s'
                     % pt_id).data
...
def test_get_all_tests_with_sort(admin):
    ...
    t_3 = admin.post('/api/v1/tests',
                     data={'name': 'pname3'}).data['test']

    gts = admin.get('/api/v1/tests?sort=created_at').data
    assert gts['tests'] == [t_1, t_2, t_3]

    # test in reverse order
    gts = admin.get('/api/v1/tests?sort=-created_at').data
    assert gts['tests'] == [t_3, t_2, t_1]
