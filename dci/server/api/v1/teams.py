...
def get_all_teams(user_info):
    ...
    user_info_team_id = user_info.get('team_id')
    if user_info_team_id != dci_config.TEAM_ADMIN_ID:
        query = query.where(models.TEAMS.c.team_id == user_info_team_id)
...
@auth2.requires_auth
def get_team_by_id_or_name(t_id, user_info):
...
@auth2.requires_auth
def get_remotecis_by_team(team_id, user_info):
...
@auth2.requires_auth
def delete_team_by_id_or_name(ct_id, user_info):
    # get If-Match header
    if_match_etag = utils.check_and_get_etag(flask.request.headers)
