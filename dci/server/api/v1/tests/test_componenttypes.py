...
def test_create_componenttypes(test_client):
    ...
    pct_id = json.loads(pct)['id']
    gct = test_client.get('/api/v1/componenttypes/%s' % pct_id).data
    gct = json.loads(gct)

    assert gct['name'] == 'pname'


def test_create_componenttypes_missing_params(test_client):
    pstatus_code = test_client.post('/api/v1/componenttypes',
                                    data={}).status_code
    assert pstatus_code == 422

    pstatus_code = test_client.post('/api/v1/componenttypes').status_code
    assert pstatus_code == 422
