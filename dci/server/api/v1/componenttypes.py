...
from dci.server.common.schemas import component_type
...
def create_componenttypes():
    ...
    result = component_type({'id': result.inserted_primary_key[0]})
...
def get_all_componenttypes():
    ...
    try:
        ...
        raise exceptions.APIException(str(e), status_code=500)
...
def get_componenttype_by_id_or_name(ct_id):
    ...
    print result
    if result is None:
        raise exceptions.APIException("Component type '%s' not found." % ct_id,
                                      status_code=404)

    result = component_type(dict(result))
    result = json.dumps(result, default=utils.json_encoder)
    return flask.Response(result, 200, content_type='application/json')
...
def delete_componenttype_by_id_or_name(ct_id):
    ...
    if result.rowcount == 0:
        raise exceptions.APIException("Conflict on component type '%s'." %
