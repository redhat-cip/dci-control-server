...
class DCIGrafanaEngine(object):
    ...
    def __init__(self, conf):
        self.host = conf['GRAFANA_HOST']
        self.port = conf['GRAFANA_PORT']
        self.user = conf['GRAFANA_USER']
        self.password = conf['GRAFANA_PASS']
        self.influxdb_host = conf['INFLUXDB_HOST']
        self.influxdb_port = conf['INFLUXDB_PORT']
        self.influxdb_admin_user = conf['INFLUXDB_USER']
        self.influxdb_admin_password = conf['INFLUXDB_PASS']
    ...
    def create_datasource(self, name):
        ...
        payload = {
            'name': name,
            'type': 'influxdb',
            'url': 'http://%s:%s' % (self.influxdb_host, self.influxdb_port),
            'access': 'direct',
            'database': name,
            'user': self.influxdb_admin_user,
