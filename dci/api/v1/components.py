...
def purge_archived_components(user):
    ...
    q_archived_components = v1_utils.QueryBuilder(_TABLE)
    q_archived_components.add_extra_condition(_TABLE.c.state == 'archived')
    archived_components = q_archived_components.execute(fetchall=True,
                                                        use_labels=False)
