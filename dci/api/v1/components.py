...
_EMBED_MANY = {"files": True, "jobs": True}
...
def _verify_component_and_topic_access(user, component):
    ...
    if component_team_id is not None:
        ...
        topic = v1_utils.verify_existence_and_get(component["topic_id"], models.TOPICS)
...
def update_components(user, c_id):
    ...
    where_clause = sql.and_(_TABLE.c.etag == if_match_etag, _TABLE.c.id == c_id)
    ...
    return flask.Response(
        json.dumps({"component": result.fetchone()}),
        200,
...
def get_all_components(user, topic_id):
    ...
    if user.is_not_super_admin() and user.is_not_feeder() and user.is_not_epm():
        query.add_extra_condition(
            sql.or_(_TABLE.c.team_id.in_(user.teams_ids), _TABLE.c.team_id == None)  # noqa
        )
...
def list_components_files(user, c_id):
    ...
    nb_rows = query.get_number_of_rows(
        models.COMPONENTFILES, models.COMPONENTFILES.c.component_id == c_id
    )
    rows = query.execute(fetchall=True)
    rows = v1_utils.format_result(rows, models.COMPONENTFILES.name, None, None)

    return flask.jsonify({"component_files": rows, "_meta": {"count": nb_rows}})
...
def get_component_file(user, c_id, f_id):
    ...
    where_clause = sql.and_(
        COMPONENT_FILES.c.id == f_id, COMPONENT_FILES.c.component_id == c_id
    )
...
def download_component_file(user, c_id, f_id):
    ...
    component_file = v1_utils.verify_existence_and_get(f_id, models.COMPONENT_FILES)
...
def verify_and_get_components_ids(
    topic_id, components_ids, component_types, db_conn=None
):
...
def add_tag_to_component(user, c_id):
    ...
    if tag_name and tag_name not in component["tags"]:
        ...
        query = _TABLE.update().where(_TABLE.c.id == c_id).values(**cmpt_values)
...
def purge_archived_components(user):
    ...
    for cmpt in archived_components:
        ...
        for cmpt_file in cmpt_files:
            ...
            try:
                q_delete_cfile = models.COMPONENT_FILES.delete().where(
                    models.COMPONENT_FILES.c.id == cmpt_file["id"]
                )
        ...
        flask.g.db_conn.execute(_TABLE.delete().where(_TABLE.c.id == cmpt["id"]))
