...
def create_components(user):
    ...
    if "team_id" in values and "topic_id" in values:
        raise dci_exc.DCIException("either team_id or topic_id, not both")
    if "team_id" not in values and "topic_id" not in values:
        raise dci_exc.DCIException("either team_id or topic_id required")
...
def get_all_components(user, topic_id=None, team_id=None):
