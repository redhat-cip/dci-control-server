...
def update_product(user, product_id):
    ...
    where_clause = sql.and_(_TABLE.c.etag == if_match_etag, _TABLE.c.id == product_id)
    query = (
    ...
    return flask.Response(
        json.dumps({"product": result.fetchone()}),
        200,
        headers={"ETag": values["etag"]},
        content_type="application/json",
    )
...
def delete_team_from_product(user, product_id, team_id):
    ...
    where_clause = sql.and_(JPT.c.product_id == product["id"], JPT.c.team_id == team_id)
