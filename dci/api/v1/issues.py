...
def _get_or_create_issue(data):
    ...
    try:
        ...
        query = sql.select([_TABLE]).where(
            sql.and_(
                _TABLE.c.url == values["url"], _TABLE.c.topic_id == values["topic_id"]
            )
        )
        return flask.g.db_conn.execute(query).fetchone()
...
def get_issues_by_resource(resource_id, table):
    ...
    if table.name == "jobs":
        ...
        j2 = sql.join(
            _TABLE,
            JJI,
            sql.and_(_TABLE.c.id == JJI.c.issue_id, JJI.c.job_id == resource_id),
        )
    ...
    return flask.jsonify({"issues": rows, "_meta": {"count": len(rows)}})
...
def unattach_issue(resource_id, issue_id, table):
    ...
    if table.name == "jobs":
        ...
        where_clause = sql.and_(
            join_table.c.job_id == resource_id, join_table.c.issue_id == issue_id
        )
    else:
        join_table = models.JOIN_COMPONENTS_ISSUES
        where_clause = sql.and_(
            join_table.c.component_id == resource_id, join_table.c.issue_id == issue_id
        )
...
def attach_issue(resource_id, table, user_id):
    ...
    query = join_table.insert().values(
        {"user_id": user_id, "issue_id": issue["id"], key: resource_id}
    )
...
def create_issue(user):
    ...
    return flask.Response(
        result, 201, headers={"ETag": issue["etag"]}, content_type="application/json"
    )
...
def delete_issue_by_id(user, issue_id):
    ...
    with flask.g.db_conn.begin():
        ...
        where_clause = sql.and_(_TABLE.c.etag == if_match_etag, _TABLE.c.id == issue_id)
...
def add_test_to_issue(user, issue_id):
    ...
    v1_utils.verify_existence_and_get(values.get("test_id"), models.TESTS, get_id=True)
    q_insert = models.JOIN_ISSUES_TESTS.insert().values(**values)
    flask.g.db_conn.execute(q_insert)
    return flask.Response(json.dumps(values), 201, content_type="application/json")
...
def remove_test_from_issue(users, issue_id, test_id):
    ...
    query = _JIT.delete().where(
        sql.and_(_JIT.c.issue_id == issue_id, _JIT.c.test_id == test_id)
    )
...
def get_tests_from_issue(user, issue_id):
    ...
    return flask.Response(
        json.dumps({"tests": results}), 200, content_type="application/json"
    )
