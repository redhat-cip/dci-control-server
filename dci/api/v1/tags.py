...
def create_tag(join_table, object_id, values):
...
def get_all_tags_from_object(join_table, object_id):
...
def get_tag_from_object(join_table, object_id, tag_id):
    """Get specific tag from a specific object."""
...
def delete_tag(join_table, object_id, tag_id):
    """Delete a tag from a specific object."""

    tag_retrieved = v1_utils.verify_existence_and_get(tag_id, table)

    if str(tag_retrieved['object_id']) != object_id:
        raise dci_exc.DCIDeleteConflict(
            "Tag '%s' is not associated to object '%s'." % (tag_id, object_id))

    values = {'state': 'archived'}

    query = table.update().where(table.c.id == tag_id).values(**values)
