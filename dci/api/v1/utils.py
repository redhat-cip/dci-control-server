...
Embed = collections.namedtuple('Embed', [
    'model', 'many', 'select', 'where', 'sort', 'join'])


def embed(model=None, many=False, select=None, where=None,
          sort=None, join=None):
    return Embed(model, many, select, where, sort, join)
...
class QueryBuilder(object):
    ...
    def join(self, embed_list):
        ...
        for embed in sorted(set(embed_list)):
            ...
            if e.model is not None:
                j = self.table.outerjoin(e.model)
                self._join.append(j)
    ...
    def build_list_ids(self):
        query = sql.select([func.distinct(self.table.c.id)])
