...
def group_reduce(rows, items_to_embed):
    ...
    def unique_rows_ids(unique_rows):
        ids = []
    ...
    # TODO (spredzy) This function does not keep ordering.
    #  unique = [dict(i) for i in set([frozenset(i.items()) for i in rows])]
    unique_rows = []
    for row in rows:
        if row['id'] not in unique_rows_ids(unique_rows):
    ...
    for unique_row in unique_rows:
        for item in items_to_embed:
            if len(unique_row[item + 's']) == 1:
                del unique_row[item + 's']
