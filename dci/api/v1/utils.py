...
def get_root_team(self, team_id):
    ...
    while True:
        ...
        if not result['parent_id']:
            return result['id']
        if result['id'] in seen_teams:
            raise dci_exc.DCIException('Team ancestry malformed')
        seen_teams.append(result['id'])
...
def verify_team_in_topic(user, topic_id):
    ...
    """
    if auth.is_admin(user) or user.is_read_only_user():
        return
...
def check_export_control(user, topic):
    ...
    if topic['export_control']:
        product = verify_existence_and_get(topic['product_id'],
                                                    models.PRODUCTS)
        if get_root_team(user.team_id) == product['team_id']:
            return
    else:
