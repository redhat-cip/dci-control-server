...
def _build_new_template(topic_id, remoteci, jd_type, values):
    # Get the jobdefinition according to the type provided above
    q_jd = (sql.select([models.JOBDEFINITIONS])
            .where(models.JOBDEFINITIONS.c.type == jd_type))
    jd_to_run = flask.g.db_conn.execute(q_jd).fetchone()

    if jd_to_run is None:
        msg = 'Jobdefinition type "%s" not found.' % jd_type
...
def schedule_jobs(user):
    ...
    if recheck_job:
        ...
        values = _build_new(topic_id, remoteci, values)
...
@api.route('/jobs/job_schedule_template', methods=['POST'])
@auth.requires_auth
def job_schedule_template(user):
    """Dispatch jobs to remotecis.
    ...
    if recheck_job:
        ...
        values = _build_new_template(topic_id, remoteci, jd_type, values)
