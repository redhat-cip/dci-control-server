...
def create_new_update_job_from_an_existing_job(user, job_id):
    ...
    original_job = v1_utils.verify_existence_and_get(original_job_id, models.JOBS)
...
def create_new_upgrade_job_from_an_existing_job(user):
    ...
    values = _build_job(
        product_id, next_topic_id, remoteci, [], values, previous_job_id=original_job_id
    )
...
def add_tag_to_job(user, job_id):
    ...
    return flask.Response(json.dumps(job), 201, content_type="application/json")
