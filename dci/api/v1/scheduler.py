...
def get_last_rconfiguration_id(topic_id, remoteci_id):
...
def get_remoteci_configuration(topic_id, remoteci_id):
...
def get_component_types(topic_id, remoteci_id):

    rconfiguration = get_remoteci_configuration(topic_id, remoteci['id'])
    ...
    if (rconfiguration is not None and
        ...
        component_types = scheduler.get_component_types_from_topic(topic_id)
...
def verify_and_get_components_ids(topic_id, component_types, components_ids):
    if components_ids == []:
        schedule_components_ids = scheduler.get_last_components(
            component_types, topic_id)
    else:
        schedule_components_ids = scheduler.get_components_from_ids(
            topic_id, components_ids, component_types)

def validate_input(values, identity):
