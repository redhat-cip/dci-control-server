...
_EMBED_MANY = {"jobstate": False, "jobstate.job": False, "job": False, "team": False}
...
def get_previous_job_in_topic(job):
    ...
    query = (
        ...
        .where(
            sql.and_(
                models.JOBS.c.topic_id == topic_id,
...
def _get_previous_jsonunit(job, filename):
    ...
    query = sql.select([models.TESTS_RESULTS]).where(
        sql.and_(
            models.TESTS_RESULTS.c.job_id == prev_job["id"],
...
def create_files(user):
    ...
    if values.get("jobstate_id") and values.get("job_id") is None:
        jobstate = v1_utils.verify_existence_and_get(
            values.get("jobstate_id"), models.JOBSTATES
        )
