...
def create_users(user):
    ...
    if not user.is_super_admin() and \
       not (user.is_product_owner() and
            user.is_in_team(values['team_id'])) and \
       not (user.is_admin() and user.is_in_team(values['team_id'])):
...
def put_user(user, user_id):
    ...
    if puser['id'] != str(user_id):
        if not user.is_super_admin() and \
           not (user.is_product_owner() and
                user.is_in_team(puser['team_id'])) and \
           not (user.is_admin() and user.is_in_team(puser['team_id'])):
            raise auth.UNAUTHORIZED
...
def delete_user_by_id(user, user_id):
    ...
    if not user.is_super_admin() and \
       not (user.is_product_owner() and
            user.is_in_team(duser['team_id'])) and \
       not (user.is_admin() and user.is_in_team(duser['team_id'])):
        raise auth.UNAUTHORIZED
