...
def create_users(user):
    ...
    try:
        session.add(new_user, _warn=True)
        session.commit()
        session.flush()
...
def get_user_by_id_or_name(user, user_id):
    ...
    if not user.is_team_admin(user):
        query.filter(dci_orm.User.team_id == user.team_id)
...
def put_user(user, user_id):
    ...
    puser = _verify_existence_and_get_user(user_id)
    ...
    if values['team_id'] and not user.is_super_admin():
        raise auth.UNAUTHORIZED

    if values['role'] and not user.is_team_admin(puser.team_id):
        raise auth.UNAUTHORIZED
