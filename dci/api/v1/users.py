...
def get_all_users(user, team_id=None):
    ...
    # Get all informations about the user who is making the request
    cur_user = query.join('team').filter(users.User.id == user['id']).first()

    # If the user is not super admin limit the select to his team
    if not cur_user.is_super_admin():
        query = query.filter(users.User.team_id == cur_user.team_id)
