...
def get_jobs_status_from_components(user, topic_id, type_id):
    ...
    EMBED = {
        'jobdefinitions': v1_utils.embed(models.JOBDEFINITIONS),
        'teams': v1_utils.embed(models.TEAMS),
        'topics': v1_utils.embed(models.TOPICS),
        'jobs_components': v1_utils.embed(models.JOIN_JOBS_COMPONENTS),
        'teams_topics': v1_utils.embed(models.JOINS_TOPICS_TEAMS),
        'remotecis': v1_utils.embed(models.REMOTECIS),
        'components': v1_utils.embed(models.COMPONENTS),
    }


    # Get the last component id of <type_id> type within <topic_id> topic
    ...
    q_bd = sql.select([models.COMPONENTS.c.id]).where(where_clause).order_by(
    ...
    # rcs = [ rc['job'] = '' for rc in rcs ]

    # Get status of last job with last component for all remotecis for a
    jjc = models.JOBS.join(models.JOIN_JOBS_COMPONENTS, models.JOBS.c.id == models.JOIN_JOBS_COMPONENTS.c.job_id).join(models.REMOTECIS, models.JOBS.c.remoteci_id == models.REMOTECIS.c.id)
    q_bd = sql.select([models.JOBS]).select_from(jjc).distinct(models.REMOTECIS.c.id).where(models.JOIN_JOBS_COMPONENTS.c.component_id == cpt_id)
