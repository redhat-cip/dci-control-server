...
def put_topic(user, topic_id):
    ...
    if next_topic:
        next_topic = v1_utils.verify_existence_and_get(next_topic,
                                                       _TABLE,
                                                       get_id=True)
        # verify user's team in the next_topic
        v1_utils.verify_team_in_topic(user, next_topic)
...
def get_jobs_status_from_components(user, topic_id, type_id):
    ...
    return flask.jsonify({'jobs': to_return,
                          '_meta': {'count': len(to_return)}})
