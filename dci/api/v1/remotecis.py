...
@api.route('/remotecis/<remoteci_id>', methods=['GET'])
@auth.requires_auth
def get_remoteci_by_id_or_name(user, remoteci_id):
    ...
    try:
        uuid.UUID(remoteci_id)
        query.add_extra_condition(_TABLE.c.id == remoteci_id)
    except ValueError:
        query.add_extra_condition(_TABLE.c.name == remoteci_id)
    ...
    return flask.jsonify({'remoteci': rows[0], '_meta': {'count': len(rows)}})
...
@api.route('/remotecis/<remoteci_id>', methods=['DELETE'])
@auth.requires_auth
