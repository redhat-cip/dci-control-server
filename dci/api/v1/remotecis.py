...
_EMBED_MANY = {"team": False, "users": True}
...
def get_all_remotecis(user, t_id=None):
    ...
    query = v1_utils.QueryBuilder(
        _TABLE, args, _R_COLUMNS, ignore_columns=["keys", "cert_fp"]
    )
...
def get_remoteci_by_id(user, r_id):
    ...
    return base.get_resource_by_id(
        user, remoteci, _TABLE, _EMBED_MANY, ignore_columns=["keys", "cert_fp"]
    )
...
def delete_remoteci_by_id(user, remoteci_id):
    ...
    with flask.g.db_conn.begin():
        ...
        where_clause = sql.and_(
            _TABLE.c.etag == if_match_etag, _TABLE.c.id == remoteci_id
        )
        ...
        for model in [models.JOBS]:
            query = (
                model.update()
                .where(model.c.remoteci_id == remoteci_id)
                .values(**values)
            )
            flask.g.db_conn.execute(query)
...
def add_user_to_remoteci(user, r_id):
    ...
    query = models.JOIN_USER_REMOTECIS.insert().values(
        {"user_id": user.id, "remoteci_id": r_id}
    )
    try:
        flask.g.db_conn.execute(query)
    except sa_exc.IntegrityError:
...
def delete_user_from_remoteci(user, r_id, u_id):
    ...
    JUR = models.JOIN_USER_REMOTECIS
    where_clause = sql.and_(JUR.c.remoteci_id == r_id, JUR.c.user_id == u_id)
...
def update_remoteci_keys(user, r_id):
    ...
    values["etag"] = utils.gen_etag()
    values["cert_fp"] = re.sub(":", "", cert.digest("sha1").decode("utf-8")).lower()
    ...
    return flask.Response(
        json.dumps({"keys": keys}), 201, content_type="application/json"
    )
