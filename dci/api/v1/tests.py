...
def create_tests(user):
    ...
    return flask.Response(
        json.dumps({"test": values}), 201, content_type="application/json"
    )
...
def update_tests(user, t_id):
    ...
    where_clause = sql.and_(_TABLE.c.etag == if_match_etag, _TABLE.c.id == t_id)
    ...
    return flask.Response(
        json.dumps({"test": result.fetchone()}),
        200,
...
def get_tests_to_issues(topic_id):
    query = (
        sql.select([models.TESTS, models.ISSUES], use_labels=True)
        .select_from(models.TESTS.join(models.JOIN_ISSUES_TESTS).join(models.ISSUES))
    ...
    for tji in tests_join_issues:
        ...
        issue = {"id": str(tji["issues_id"]), "url": tji["issues_url"]}
