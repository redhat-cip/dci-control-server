...
def verify_repo_access(user):
    ...
    if verify != "SUCCESS":
        return flask.Response("wront SSLVerify header", 403)

    if len(splitpath(url)) < 3:
        return flask.Response(
            "Requested url is invalid should be /<product_id>/<topic_id>/<component_id>",
            403,
        )
    ...
    if not remoteci:
        return flask.Response("SSL fingerprint is not good", 403)

    team_id = remoteci["team_id"]
    if not team_has_access_to_a_topic(team_id, product_id, topic_id):
        return flask.Response(
            "Team %s don't have access to the topic %s" % (team_id, topic_id), 403
        )
