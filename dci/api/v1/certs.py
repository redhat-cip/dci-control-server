...
def verify_repo_access(user):
    ...
    if str(topic['product_id']) != str(product_id):
        return flask.Response(
            'topic %s/%s does not belongs to product %s/%s' % (topic['name'], topic['id'], product['name'], product['id']),  # noqa
            403)
    ...
    if str(component['topic_id']) != str(topic_id):
        return flask.Response(
            'component %s/%s does not belongs to topic %s/%s' % (component['name'], component['id'], topic['name'], topic['id']),  # noqa
            403)
