...
def _get_teams_of_user(user, user_id):
    ...
    nb_user_teams = query.get_number_of_rows()
    ...
    # second, get child teams
    query = v1_utils.QueryBuilder(models.TEAMS, args, _T_COLUMNS)
    query.add_extra_condition(models.TEAMS.c.state != 'archived')
    query.add_extra_condition(models.TEAMS.c.id.in_(user.child_teams_ids))
    nb_child_teams = query.get_number_of_rows()
    rows = query.execute(fetchall=True)
    child_teams = v1_utils.format_result(rows, models.TEAMS.name,
                                         args['embed'],
                                         _EMBED_MANY)
    # user is the product owner of each child team
    for ct in child_teams:
        ct['role'] = 'PRODUCT_OWNER'
    users_teams.extend(child_teams)

    return users_teams, nb_user_teams + nb_child_teams
...
def get_teams(user):
    if user.is_not_super_admin():
        user_teams, nb_teams = _get_teams_of_user(user, user.id)
        ...
        admin_teams = query.execute(fetchall=True)
        admin_teams = v1_utils.format_result(admin_teams, _TABLE.name,
                                             args['embed'],
                                             _EMBED_MANY)
        for team in admin_teams:
            team['role'] = 'SUPER_ADMIN'
        return flask.jsonify({'teams': admin_teams,
                              '_meta': {'count': nb_teams}})
