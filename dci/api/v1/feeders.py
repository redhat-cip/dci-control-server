...
def get_feeder_by_id(user, f_id):
    ...
    if not user.is_super_admin() and \
       not user.is_team_product_owner(feeder['team_id']):
        raise auth.UNAUTHORIZED
...
@api.route('/feeders/<uuid:f_id>/data', methods=['GET'])
@decorators.login_required
def get_feeder_data(user, f_id):
    query = v1_utils.QueryBuilder(_TABLE, {}, _F_COLUMNS)

    if not user.is_super_admin():
        query.add_extra_condition(_TABLE.c.team_id.in_(user.teams))

    query.add_extra_condition(_TABLE.c.id == f_id)
    row = query.execute(fetchone=True)

    if row is None:
        raise dci_exc.DCINotFound('Feeder', f_id)
...
def put_api_secret_feeder(user, f_id):
    ...
    res = flask.jsonify(({'id': f_id, 'etag': values['etag'],
