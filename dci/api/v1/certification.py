...
def build_certification(username, password, certification_id, file_name,
                        file_content):
    return {
        'username': username,
        'password': password,
...
def send_certification(user):
    ...
    file_object = get_file_object(data['file_id'])
    ...
    try:
        ...
        raise exceptions.DCIException('Cannot upload certification result',
                                      status_code=400)
