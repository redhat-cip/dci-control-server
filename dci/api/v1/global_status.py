...
SELECT DISTINCT ON (jobs.remoteci_id, jobs.rconfiguration_id) remotecis.name as remoteci_name, teams.name as team_name, rconfigurations.name as rconfiguration_name, components.name, components.id, jobs.id, jobs.status, jobs.created_at, jobs.remoteci_id
FROM jobs
...
WHERE teams.external = '1' AND (jobs.status = 'failure' OR jobs.status = 'success') AND remotecis.state = 'active' AND components.id = '%s' ORDER BY jobs.remoteci_id, jobs.rconfiguration_id, jobs.created_at DESC
        """ % component['id'])  # noqa
