...
SELECT DISTINCT ON (jobs.remoteci_id) remotecis.name as remoteci_name, teams.external, components.name, components.id, jobs.id, jobs.status, jobs.created_at, jobs.remoteci_id
FROM jobs
...
WHERE teams.external = '1' AND remotecis.state = 'active' AND components.id = '%s' ORDER BY jobs.remoteci_id, jobs.created_at DESC
        """ % component['id'])  # noqa
